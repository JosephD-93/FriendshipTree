...
const QUERY_ENGINE_VERSION = "3.1.1";
...
function overview() {
  ...
  for (const relation of db.relationships) {
    const source = relation?.sourceOwner || "Unknown";
    const target = relation?.targetOwner || "Unknown";
    const type = relation?.relationType || "reference";
    const key = `${source}\u0000${target}\u0000${type}`;
    relationshipMap.set(key, (relationshipMap.get(key) || 0) + 1);
  }
  const strongestRelationships = [...relationshipMap.entries()]
    .map(([key,count]) => {
  ...
  const resolvedCount = db.relationships.filter(r=>r.resolution==="resolved-local").length;
  const localTotal = resolvedCount + unresolvedCount;
  return {
    ok:true,
    queryEngineVersion:QUERY_ENGINE_VERSION,
    queryEnginePath:__filename,
    databasePath,
    databaseBytes:stat.size,
    databaseModifiedAt:stat.mtime.toISOString(),
    schemaVersion:db.schemaVersion,
    engineVersion:db.engineVersion || null,
    scanId:db.scanId || null,
    startedAt:db.startedAt || null,
    completedAt:db.completedAt || null,
    storageFormat:db.storageFormat || "Project Brain",
    totals:{
      files:base.files,
      directories:base.directories,
      relationships:base.relationships,
      components:base.components,
      unknownFiles:safeNumber(base.filesByOwner.Unknown),
      resolved:resolvedCount,
      unresolved:unresolvedCount
    },
    byOwner:base.filesByOwner,
    byState:base.filesByState,
    resolutionRate:localTotal ? Math.round((resolvedCount/localTotal)*1000)/10 : 100,
    strongestRelationships,
    components:(db.components || []).map(component => ({
      id:component.id,
      name:component.name,
...
function exploreFile(query, depth=3) {
  const details = fileDetails(query);
  if (!details.ok) return details;
  const deps = dependencies(details.file.path);
  const users = dependents(details.file.path);
  const affected = impact(details.file.path, depth);
  const affectedCount = safeNumber(affected.totalAffected);
  return {
    ok:true,
    queryEngineVersion:QUERY_ENGINE_VERSION,
    file:details.file,
    dependencies:deps.ok ? deps.dependencies.map(r=>({
      sourcePath:details.file.path,
      targetPath:r.targetPath,
      sourceOwner:details.file.owner || "Unknown",
      targetOwner:r.targetOwner || "Unknown",
    ...
    dependents:users.ok ? users.dependents.map(r=>({
      sourcePath:r.sourcePath,
      targetPath:details.file.path,
      sourceOwner:r.sourceOwner || "Unknown",
      targetOwner:details.file.owner || "Unknown",
      relationType:r.relationType || "reference",
      specifier:r.specifier || "",
      resolution:r.resolution || "",
      lineNumber:r.lineNumber || null,
    ...
    impact:{
      ...
      risk:affectedCount >= 25 ? "HIGH" : affectedCount >= 8 ? "MEDIUM" : "LOW",
      levels:affected.ok ? affected.levels.slice(1) : []
    }
  };
}
...
function execute(name,args={}) {
  switch(String(name||"").toLowerCase()){
    ...
    case "explore-file": case "explore": return exploreFile(args.query,safeNumber(args.depth)||3);
...
Commands:
  ...
  overview
  file <path-or-name>
  explore-file <path-or-name> [--depth 3]
  deps <path-or-name>
...
function positionalAfter(cmd) {
  ...
  // Command arguments are positional only until the first option.
  // The previous parser treated --json as though it had a value and
  // accidentally appended --project-root's value to the search term.
  // Example: "search app --limit 20 --json --project-root C:\\..."
  // became the query "app C:\\...", which correctly returned no matches.
  const vals = [];
