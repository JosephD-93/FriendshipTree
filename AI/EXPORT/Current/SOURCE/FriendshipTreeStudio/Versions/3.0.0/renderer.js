...
async function initialiseStudioVersion() {
  const label = $('studioVersion');
  try {
    const info = await window.studio.appInfo();
    const rawVersion = String(info?.version || '2.8.0');
    const displayVersion = rawVersion.replace(/\.0$/, '');
    if (label) label.textContent = 'V' + displayVersion;
    document.title = `FriendshipTree Studio V${displayVersion}`;
  } catch (error) {
    if (label) label.textContent = 'V2.8';
    document.title = 'FriendshipTree Studio V2.8';
    console.error('Could not read Studio version:', error);
  }
}
initialiseStudioVersion();
...
$('openUpdateInbox').onclick = () => window.studio.openUpdateInbox();
$('scanUpdateInbox').onclick = async () => {
