...
export default function WellbeingStack(){
 const [angle,setAngle]=useState(START); const scrollRef=useRef(null);
 const layout=useMemo(()=>WELLBEING_NODES.map((node,i)=>({...node,_i:i,...projectNodeToStack(node,angle)})).filter(n=>n.visible),[angle]);
 const nearest=WELLBEING_DOMAINS.slice().sort((a,b)=>Math.abs(projectNodeToStack({...a,primaryDomain:a.id,secondaryDomains:[]},angle).delta)-Math.abs(projectNodeToStack({...b,primaryDomain:b.id,secondaryDomains:[]},angle).delta))[0];
 ...
 return <section style={{padding:"8px 8px 110px",maxWidth:760,margin:"0 auto",overflow:"hidden"}}>
  ...
  <div ref={scrollRef} onScroll={onScroll} style={{height:"72vh",minHeight:440,overflowY:"auto",overscrollBehavior:"contain",touchAction:"pan-y",scrollbarWidth:"none",position:"relative"}}>
   <div style={{height:"260vh",position:"relative"}} aria-hidden="true"/>
   <div style={{position:"sticky",top:0,height:"72vh",minHeight:440,marginTop:"-260vh",pointerEvents:"none"}}>
    ...
    {layout.map(node=>{
     const influences=[node.primaryDomain,...(node.secondaryDomains||[])],colour=mix(influences),shared=influences.length>1;
     // Side is stable for each node. Domain overlap affects vertical motion only.
     const side=node._i%2?1:-1,x=49+side*(shared?34:38);
     const verticalScale=shared?18*node.parallax:24;
     const laneOffset=((Math.floor(node._i/2)%3)-1)*7;
     const y=clamp(50+node.delta*verticalScale+laneOffset,6,94);
  ...
  <div style={{textAlign:"center",fontSize:11,opacity:.48,padding:"8px 14px"}}>Shared nodes keep their side and move more slowly vertically, remaining between the sections they connect.</div>
 </section>;
}
