...
function runAIWorkspace(mode) {
  const script = path.join(PROJECT_ROOT, "FriendshipTreeLauncher", "Manage-AI-Workspace.ps1");
  if (!fs.existsSync(script)) throw new Error("AI Workspace manager is missing.");
  startPowerShell(["-File", script, "-Mode", mode], false);
}
ipcMain.handle("ai-workspace:refresh", () => runAIWorkspace("Refresh"));
ipcMain.handle("ai-workspace:validate", () => runAIWorkspace("Validate"));
ipcMain.handle("ai-workspace:requested-file", () => runAIWorkspace("RequestedFile"));
ipcMain.handle("ai-workspace:open", () => runAIWorkspace("Open"));
ipcMain.handle("ai-workspace:copy-instructions", () => runAIWorkspace("CopyInstructions"));
