...
function Get-ForgeState {
    ...
    if ($null -eq $State.PSObject.Properties["forgeVersion"]) {
        $State | Add-Member -NotePropertyName forgeVersion -NotePropertyValue "2.1.2"
    } else {
        $State.forgeVersion = "2.1.2"
    }
    return $State
}
...
function Start-StudioVersion {
    ...
    if (!$Electron) {
        [System.Windows.Forms.MessageBox]::Show("Electron could not be found.","FriendshipTree Forge") | Out-Null
        return $false
    }
...
function Confirm-CurrentWorking {
    $State = Get-ForgeState
    if (!$State.pending) {
        [System.Windows.Forms.MessageBox]::Show("No candidate is awaiting confirmation.","FriendshipTree Forge") | Out-Null
        return
    }
...
function Rollback-Current {
    ...
    if (!$Target) {
        [System.Windows.Forms.MessageBox]::Show("No rollback version is available.","FriendshipTree Forge") | Out-Null
        return
    }
...
$Form.Text = "FriendshipTree Forge 2.1.2"
...
$Title.Text = "FriendshipTree Forge 2.1.2"
...
$script:CurrentLabel.Location = New-Object System.Drawing.Point(28,65)
...
$script:GoodLabel.Location = New-Object System.Drawing.Point(28,88)
...
$script:PendingLabel.Location = New-Object System.Drawing.Point(28,111)
...
$script:InboxLabel.Location = New-Object System.Drawing.Point(28,134)
$Form.Controls.Add($script:InboxLabel)

$script:UpdateList = New-Object System.Windows.Forms.ListView
$script:UpdateList.Location = New-Object System.Drawing.Point(28,170)
$script:UpdateList.Size = New-Object System.Drawing.Size(910,155)
...
Add-Button "Install selected update" 28 337 210 {
...
Add-Button "Refresh" 250 337 120 { Update-ForgeDisplay }
Add-Button "Create workflow test package" 382 337 240 { Build-WorkflowTestPackage }
Add-Button "Open Inbox" 634 337 140 { Start-Process explorer.exe -ArgumentList @($InboxRoot) }
Add-Button "Open logs" 786 337 152 { Start-Process explorer.exe -ArgumentList @($LogsRoot) }

$script:VersionGrid = New-Object System.Windows.Forms.DataGridView
$script:VersionGrid.Location = New-Object System.Drawing.Point(28,395)
$script:VersionGrid.Size = New-Object System.Drawing.Size(910,180)
...
Add-Button "Launch current" 28 590 150 {
    $State = Get-ForgeState
    if (!(Start-StudioVersion ([string]$State.current))) { Rollback-Current }
}
Add-Button "Launch last working" 190 590 170 {
    $State = Get-ForgeState
    [void](Start-StudioVersion ([string]$State.lastGood))
}
Add-Button "Confirm candidate works" 372 590 190 { Confirm-CurrentWorking }
Add-Button "Roll back" 574 590 150 { Rollback-Current }
Add-Button "Open Versions" 736 590 202 { Start-Process explorer.exe -ArgumentList @($VersionsRoot) }
