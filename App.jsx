      ...
      <div className={`w-80 border-r shadow-xl flex flex-col z-10 transition-all duration-300 ${sidebarBg} ${selectedNode || showTutorial ? 'translate-x-0' : '-translate-x-full'}`} style={{ marginLeft: selectedNode || showTutorial ? 0 : '-20rem' }}>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {selectedNode ? (
            ...
            <div className={`p-5 rounded-2xl border space-y-4 ${theme.darkMode ? 'bg-indigo-900/30 border-indigo-800' : 'bg-emerald-50 border-emerald-100'}`}>
              <div className="flex justify-between items-center">
                <h3 className="font-bold flex items-center text-emerald-600"><Info className="w-5 h-5 mr-2" /> Welcome!</h3>
                <button onClick={() => setShowTutorial(false)} className="opacity-50 hover:opacity-100"><X className="w-4 h-4" /></button>
              </div>
              <ul className="text-sm opacity-80 space-y-2 list-disc pl-4">
                <li><strong>Move:</strong> Click & drag any photo to reposition it.</li>
                <li><strong>Calendar:</strong> Toggle top-right for Birthday Timeline!</li>
                <li><strong>Edit:</strong> Double-tap any photo to open their profile.</li>
              </ul>
            </div>
          )}
        </div>
            ...
            {viewMode === 'canvas' && liftedNodeId && hexSnapPos && (() => {
              ...
              return (
                <g style={{pointerEvents:'none'}}>
                  ...
                  {/* Snap target indicator — small dot at snap centre */}
                  <circle cx={hexSnapPos.x} cy={hexSnapPos.y} r={5}
