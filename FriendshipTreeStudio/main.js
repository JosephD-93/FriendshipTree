...
const PROJECT_BRAIN_DIR = path.join(STUDIO_DATA, 'project-brain');
const PROJECT_BRAIN_DB = path.join(PROJECT_BRAIN_DIR, 'project-brain.json');
const PROJECT_BRAIN_QUERY_ENGINE = path.join(PROJECT, 'Tools', 'ProjectBrain', 'project_brain_query_engine.cjs');
...
const AI_SYNC_EXPORTS = path.join(STUDIO_DATA, 'AISyncExports');
const APP_RELEASES = path.join(STUDIO_DATA, 'AppReleases');
const UPDATE_INBOX_ROOT = path.join(PROJECT, 'StudioSystem', 'Updates');
const UPDATE_INBOX = path.join(UPDATE_INBOX_ROOT, 'Inbox');
const UPDATE_PROCESSED = path.join(UPDATE_INBOX_ROOT, 'Processed');
const UPDATE_REJECTED = path.join(UPDATE_INBOX_ROOT, 'Rejected');
...
const STUDIO_SYSTEM = path.join(PROJECT, 'StudioSystem');
const FORGE_UPDATES = path.join(STUDIO_SYSTEM, 'Updates');
const FORGE_INBOX = path.join(FORGE_UPDATES, 'Inbox');
const FORGE_STAGING = path.join(FORGE_UPDATES, 'Staging');
const DOCUMENTATION = path.join(PROJECT, 'Documentation');
const STUDIO_FORGE_HANDOVER = path.join(DOCUMENTATION, 'STUDIO-AND-FORGE-HANDOVER.md');
...
function ensureDirs() {
  [STUDIO_DATA, REPORTS, QUARANTINE, BACKUPS, TRANSACTIONS, PACKAGE_STAGING, PACKAGE_HISTORY, ELECTRON_DATA, ELECTRON_CACHE, KNOWLEDGE, MANIFEST_HISTORY, AI_PACKAGES, UPDATE_INBOX_ROOT, UPDATE_INBOX, UPDATE_PROCESSED, UPDATE_REJECTED].forEach(p => fs.mkdirSync(p, { recursive: true }));
}


function ensureStudioForgeHandover() {
  fs.mkdirSync(DOCUMENTATION, { recursive: true });
  const section = `# Studio and Forge Handover
...
`;
  fs.writeFileSync(STUDIO_FORGE_HANDOVER, section, 'utf8');
}
...
async function installInspectedPackage(inspection) {
  ...
  try {
    ...
    for (const record of manifest.files) {
      const destinationRel = normalizeRel(record.destination);
      const destination = path.join(root, destinationRel);
      const actualHash = exists(destination) && fs.statSync(destination).isFile()
        ? sha256(destination)
        : null;
      liveVerification.push({
        destination: destinationRel,
        absolutePath: destination,
        expectedHash: String(record.sha256).toLowerCase(),
        actualHash,
        verified: Boolean(actualHash && actualHash.toLowerCase() === String(record.sha256).toLowerCase())
      });
    }

    const failedLiveFiles = liveVerification.filter(file => !file.verified);
    if (failedLiveFiles.length) {
      throw new Error(`Post-install verification failed for ${failedLiveFiles.map(file => file.destination).join(', ')}`);
    }

    const installedStudioVersion = manifest.packageType === 'studio-update'
      ? readInstalledStudioVersion()
      : null;
    if (manifest.packageType === 'studio-update' && installedStudioVersion !== manifest.version) {
      throw new Error(`Installed package.json reports ${installedStudioVersion || 'no version'} instead of ${manifest.version}.`);
    }

    history.liveVerification = liveVerification;
    history.installedStudioVersion = installedStudioVersion;
    fs.writeFileSync(
      path.join(PACKAGE_HISTORY, `${safeName(manifest.packageId)}.json`),
      JSON.stringify(history, null, 2),
      'utf8'
    ...
    return {
      ok: true,
      history,
      restartRequired: manifest.packageType === 'studio-update',
      installedStudioVersion,
      verifiedLiveFiles: liveVerification.length,
      installedRoot: root,
      installedFiles: liveVerification
    };
  } catch (err) {
...
function emitUpdateInbox(status, details = {}) {
  const payload = { status, at: new Date().toISOString(), folder: UPDATE_INBOX, ...details };
  logEvent('update_inbox', status, details);
  try { mainWindow?.webContents?.send('update-inbox-event', payload); } catch {}
}

async function waitForFileStable(filePath, checks = 4, delayMs = 1000) {
  let previous = null;
  for (let i = 0; i < checks; i++) {
    ...
    if (previous !== null && current !== previous) i = 0;
    previous = current;
...
function uniqueArchivePath(folder, originalPath) {
  const parsed = path.parse(originalPath);
  let candidate = path.join(folder, parsed.base);
  let index = 1;
  while (exists(candidate)) {
...
function archiveInboxPackage(sourcePath, destinationFolder) {
  fs.mkdirSync(destinationFolder, { recursive: true });
  const target = uniqueArchivePath(destinationFolder, sourcePath);
  try {
...
async function processInboxPackage(filePath) {
  const resolved = path.resolve(filePath);
  if (updateInboxQueue.has(resolved) || !/\.ftupdate$/i.test(resolved)) return;
  updateInboxQueue.add(resolved);
  try {
    emitUpdateInbox('detected', { file: path.basename(resolved) });
    const stable = await waitForFileStable(resolved);
    if (!stable) {
      emitUpdateInbox('ignored', { file: path.basename(resolved), reason: 'File disappeared before download completed.' });
      return;
    }

    const inspection = await inspectUpdatePackage(resolved);
    if (!inspection.ok) {
      const rejected = archiveInboxPackage(resolved, UPDATE_REJECTED);
      emitUpdateInbox('rejected', {
    ...
    emitUpdateInbox('validated', {
      file: path.basename(resolved),
      packageType: inspection.manifest.packageType,
      version: inspection.manifest.version
    });

    const result = await installInspectedPackage(inspection);
    const archived = archiveInboxPackage(resolved, UPDATE_PROCESSED);
    emitUpdateInbox('installed', {
      file: path.basename(resolved),
      archivedTo: archived,
      packageType: inspection.manifest.packageType,
      version: result.installedStudioVersion || inspection.manifest.version,
      restartRequired: result.restartRequired,
      verifiedLiveFiles: result.verifiedLiveFiles || 0,
      installedRoot: result.installedRoot || null
    });

    if (result.restartRequired) {
      emitUpdateInbox('restarting', { version: inspection.manifest.version });
      setTimeout(() => { app.relaunch(); app.exit(0); }, 1200);
    }
  } catch (err) {
    let rejected = null;
    try { if (exists(resolved)) rejected = archiveInboxPackage(resolved, UPDATE_REJECTED); } catch {}
    emitUpdateInbox('failed', {
      file: path.basename(resolved),
      archivedTo: rejected,
      error: err.message
    });
  } finally {
    updateInboxQueue.delete(resolved);
  }
}

function scanUpdateInbox() {
  ensureDirs();
  let names = [];
  try { names = fs.readdirSync(UPDATE_INBOX); } catch {}
  for (const name of names) {
    if (/\.ftupdate$/i.test(name)) processInboxPackage(path.join(UPDATE_INBOX, name));
  }
}

function startUpdateInboxWatcher() {
  try {
    ensureDirs();
    if (updateInboxWatcher) return true;
    scanUpdateInbox();
    updateInboxWatcher = fs.watch(UPDATE_INBOX, { persistent: true }, (_eventType, filename) => {
      if (!filename || !/\.ftupdate$/i.test(String(filename))) return;
      setTimeout(() => processInboxPackage(path.join(UPDATE_INBOX, String(filename))), 500);
    });
    updateInboxWatcher.on('error', err => {
      try { updateInboxWatcher?.close(); } catch {}
      updateInboxWatcher = null;
      emitUpdateInbox('watcher-error', { error: err.message });
      setTimeout(() => startUpdateInboxWatcher(), 3000);
    });
    emitUpdateInbox('watching', { folder: UPDATE_INBOX });
    return true;
  } catch (err) {
    updateInboxWatcher = null;
    emitUpdateInbox('watcher-error', { error: err.message });
...
ipcMain.handle('get-update-inbox-status', () => {
  if (!updateInboxWatcher) startUpdateInboxWatcher();
  return {
    ok: true,
    folder: UPDATE_INBOX,
    processedFolder: UPDATE_PROCESSED,
    rejectedFolder: UPDATE_REJECTED,
    watching: Boolean(updateInboxWatcher)
  };
});

ipcMain.handle('open-update-inbox', () => {
  ensureDirs();
  shell.openPath(UPDATE_INBOX);
  return UPDATE_INBOX;
});

ipcMain.handle('scan-update-inbox-now', () => {
  scanUpdateInbox();
  return { ok: true, folder: UPDATE_INBOX };
});
...
function activeSourceFilesForSync() {
  let dna = null;
  try { if (exists(PROJECT_DNA)) dna = readJson(PROJECT_DNA); } catch {}
  const candidates = [];
  const pushRel = rel => {
    const clean = String(rel || '').replace(/\\/g, '/');
    const full = path.join(PROJECT, clean);
    if (clean && exists(full) && fs.statSync(full).isFile() && !candidates.includes(clean)) candidates.push(clean);
  };
  for (const rel of dna?.activeSourceFiles || dna?.activeFiles || []) {
    pushRel(typeof rel === 'string' ? rel : (rel.path || rel.file));
  }
  if (!candidates.length) {
    const fallback = ['src/App.jsx','src/main.jsx','src/services/persistence.js','src/utils/date.js',
      'package.json','vite.config.js','capacitor.config.json'];
    fallback.forEach(pushRel);
  } else {
    ['package.json','vite.config.js','capacitor.config.json'].forEach(pushRel);
  }
  return candidates;
}
...
function newestExisting(paths) {
  return paths.filter(exists).sort((a,b) => fs.statSync(b).mtimeMs - fs.statSync(a).mtimeMs)[0] || null;
}

async function runCaptured(command, args, cwd, event, eventName, extraEnv = {}) {
  logEvent(eventName, 'started', { command, args, cwd });
  const child = spawn(command, args, {
    cwd, shell: false, windowsHide: true, env: { ...process.env, ...extraEnv }
  });
  let stdout = '', stderr = '';
  child.stdout.on('data', d => { const s=d.toString(); stdout += s; event?.sender?.send('log', s); });
  child.stderr.on('data', d => { const s=d.toString(); stderr += s; event?.sender?.send('log', s); });
  return await new Promise(resolve => {
    child.on('error', err => resolve({ ok:false, code:-1, message:err.message, stdout, stderr }));
    child.on('close', code => {
      const result = { ok:code===0, code, stdout, stderr, message: code===0 ? null : (stderr.slice(-1500) || `Exited with code ${code}`) };
      logEvent(eventName, result.ok ? 'success' : 'error', { code, stdout_tail:stdout.slice(-4000), stderr_tail:stderr.slice(-4000) });
      resolve(result);
    });
  });
}

ipcMain.handle('export-ai-sync-package', async () => {
  ensureDirs();
  try {
    try { buildProjectDNA(); } catch {}
    try { buildProjectKnowledge(); } catch {}
    const stamp = new Date().toISOString().replace(/[:.]/g, '-');
    const staging = path.join(AI_SYNC_EXPORTS, `FriendshipTree-AI-Sync-${stamp}`);
    fs.rmSync(staging, { recursive:true, force:true });
    fs.mkdirSync(staging, { recursive:true });

    const sourceFiles = activeSourceFilesForSync();
    for (const rel of sourceFiles) copyFileWithParents(path.join(PROJECT, rel), path.join(staging, 'project', rel));

    const knowledgeFiles = [
      PROJECT_DNA,
      path.join(KNOWLEDGE, 'project-database.json'),
      path.join(KNOWLEDGE, 'project-rules.json'),
      path.join(KNOWLEDGE, 'Architecture.md'),
      path.join(KNOWLEDGE, 'FriendshipTree_AI_Context.md')
    ].filter(exists);
    knowledgeFiles.forEach(f => copyFileWithParents(f, path.join(staging, 'knowledge', path.basename(f))));

    const reportFiles = exists(REPORTS) ? fs.readdirSync(REPORTS)
      .filter(n => n.includes(sessionId))
      .map(n => path.join(REPORTS,n))
      .filter(exists) : [];
    reportFiles.forEach(f => copyFileWithParents(f, path.join(staging, 'diagnostics', path.basename(f))));

    const syncManifest = {
      format: 'FriendshipTree-AI-Sync',
      formatVersion: 1,
      generatedAt: new Date().toISOString(),
      studioVersion: (() => { try{return JSON.parse(fs.readFileSync(path.join(STUDIO,'package.json'),'utf8')).version}catch{return null} })(),
      projectRoot: PROJECT,
      activeSourceFiles: sourceFiles,
      knowledgeFiles: knowledgeFiles.map(path.basename),
      purpose: 'Upload this ZIP to ChatGPT so updates are generated against the current active project.'
    };
    writeJson(path.join(staging, 'AI-SYNC-MANIFEST.json'), syncManifest);

    const save = await dialog.showSaveDialog({
      defaultPath: path.join(os.homedir(), 'Desktop', `FriendshipTree-AI-Sync-${stamp}.zip`),
      filters: [{ name:'FriendshipTree AI Sync package', extensions:['zip'] }]
    });
    if (save.canceled || !save.filePath) return { canceled:true };

    const escapedStage = staging.replace(/'/g, "''");
    const escapedTarget = save.filePath.replace(/'/g, "''");
    const ps = `$ErrorActionPreference='Stop'; Compress-Archive -Path '${escapedStage}\\*' -DestinationPath '${escapedTarget}' -Force`;
    const result = await invokePowerShell(ps);
    if (!result.ok) throw new Error(result.stderr || 'Could not create AI Sync ZIP.');
    logEvent('ai_sync.export', 'success', { target:save.filePath, active_files:sourceFiles.length });
    return { ok:true, path:save.filePath, activeFiles:sourceFiles.length };
  } catch (err) {
    logEvent('ai_sync.export', 'error', { error:err.message });
    return { ok:false, message:err.message };
  }
});
...
ipcMain.handle('adb-devices', async event => {
  const tools = toolPaths();
  if (!tools.adb) return { ok:false, message:'ADB could not be found.', devices:[] };
  const result = await runCaptured(tools.adb, ['devices','-l'], PROJECT, event, 'android.adb_devices');
...
ipcMain.handle('adb-connect-wireless', async (event, address) => {
  const tools = toolPaths();
  if (!tools.adb) return { ok:false, message:'ADB could not be found.' };
  const clean = String(address || '').trim();
  if (!/^[a-zA-Z0-9._-]+:\d{2,5}$/.test(clean)) return { ok:false, message:'Enter the phone address as IP:PORT.' };
  return runCaptured(tools.adb, ['connect', clean], PROJECT, event, 'android.adb_connect');
});

ipcMain.handle('deploy-app-one-click', async event => {
  const stages = [];
  const push = (name, result) => { stages.push({name, ok:result.ok, message:result.message || null}); return result; };
  let r = push('Build web app', await runCaptured('npm.cmd', ['run','build'], PROJECT, event, 'deploy.web_build'));
  if (!r.ok) return { ok:false, stages, message:'Web build failed.' };
  r = push('Sync Android project', await runCaptured('npx.cmd', ['cap','sync','android'], PROJECT, event, 'deploy.cap_sync'));
  if (!r.ok) return { ok:false, stages, message:'Android sync failed.' };

  const tools = toolPaths();
  const gradlew = path.join(PROJECT,'android','gradlew.bat');
  if (!exists(gradlew)) return { ok:false, stages, message:'android/gradlew.bat was not found.' };
  const javaHome = tools.java ? path.dirname(path.dirname(tools.java)) : null;
  r = push('Build debug APK', await runCaptured(gradlew, ['assembleDebug'], path.join(PROJECT,'android'), event, 'deploy.apk_build', javaHome ? {JAVA_HOME:javaHome}:{}));
  if (!r.ok) return { ok:false, stages, message:'APK build failed.' };

  const apk = path.join(PROJECT,'android','app','build','outputs','apk','debug','app-debug.apk');
  if (!exists(apk)) return { ok:false, stages, message:'Build completed but app-debug.apk was not found.' };
  if (!tools.adb) return { ok:false, stages, apk, message:'APK built, but ADB was not found. Publish the APK instead.' };
  r = push('Install on connected phone', await runCaptured(tools.adb, ['install','-r',apk], PROJECT, event, 'deploy.apk_install'));
  return { ok:r.ok, stages, apk, message:r.ok ? 'FriendshipTree was built and installed successfully.' : 'APK built, but installation failed.' };
});

function googleDriveRoots() {
  const home = os.homedir();
  const envRoots = [process.env.GoogleDrive, process.env.GoogleDriveFS, process.env.OneDrive];
  const likely = [
    path.join(home,'Google Drive'),
    path.join(home,'My Drive'),
    path.join(home,'Google Drive','My Drive'),
    'G:\\My Drive', 'H:\\My Drive', 'I:\\My Drive'
  ];
  return [...envRoots, ...likely].filter(Boolean).filter(exists);
}

ipcMain.handle('publish-apk-free', async () => {
  try {
    const apk = path.join(PROJECT,'android','app','build','outputs','apk','debug','app-debug.apk');
    if (!exists(apk)) return { ok:false, message:'Debug APK not found. Run one-click deployment or Build APK first.' };
    const roots = googleDriveRoots();
    let targetFolder = roots.length ? path.join(roots[0], 'FriendshipTree Updates') : null;
    if (!targetFolder) {
      const pick = await dialog.showOpenDialog({ title:'Choose a synced folder for free APK publishing', properties:['openDirectory','createDirectory'] });
      if (pick.canceled || !pick.filePaths[0]) return { canceled:true };
      targetFolder = pick.filePaths[0];
    }
    fs.mkdirSync(targetFolder, { recursive:true });
    let appVersion = 'development';
    try {
      const p = JSON.parse(fs.readFileSync(path.join(PROJECT,'package.json'),'utf8'));
      appVersion = p.version || appVersion;
    } catch {}
    const stamp = new Date().toISOString().replace(/[:.]/g,'-');
    const filename = `FriendshipTree-${appVersion}-${stamp}.apk`;
    const target = path.join(targetFolder, filename);
    fs.copyFileSync(apk, target);
    const hash = sha256File(target);
    writeJson(path.join(targetFolder,'latest.json'), {
      version:appVersion, filename, sha256:hash, publishedAt:new Date().toISOString(),
      note:'Android will still ask the user to approve installation; this is an Android security requirement.'
    });
    logEvent('android.publish_free', 'success', { target, sha256:hash });
    shell.showItemInFolder(target);
    return { ok:true, path:target, sha256:hash };
  } catch (err) {
    logEvent('android.publish_free', 'error', { error:err.message });
    return { ok:false, message:err.message };
  }
});
...
function runProjectBrainQuery(command, query = null, options = {}) {
  return new Promise((resolve) => {
    if (!fs.existsSync(PROJECT_BRAIN_QUERY_ENGINE)) {
      resolve({
        ok: false,
        missingEngine: true,
        message: `Phase 3 Query Engine was not found: ${PROJECT_BRAIN_QUERY_ENGINE}`
      });
      return;
    }
    if (!fs.existsSync(PROJECT_BRAIN_DB)) {
      resolve({
        ok: false,
        missing: true,
        message: 'No Project Brain database was found. Run the dependency scanner first.',
        path: PROJECT_BRAIN_DB
      });
      return;
    }

    const args = [
      PROJECT_BRAIN_QUERY_ENGINE,
      command
    ];
    ...
    args.push('--json', '--project-root', PROJECT, '--database', PROJECT_BRAIN_DB);
    ...
    const timeout = setTimeout(() => {
      ...
      resolve({ ok: false, message: `Project Brain query timed out: ${command}` });
    }, 30000);
    ...
    child.on('error', error => {
      ...
      resolve({ ok: false, message: `Could not start Phase 3 Query Engine: ${error.message}` });
    });
    child.on('close', code => {
      clearTimeout(timeout);
      if (code !== 0) {
        resolve({ ok: false, message: stderr.trim() || `Query Engine exited with code ${code}.` });
        return;
      }
      try {
        resolve(JSON.parse(stdout.replace(/^\uFEFF/, '').trim()));
      } catch (error) {
        resolve({ ok: false, message: `Query Engine returned invalid JSON: ${error.message}`, detail: stdout.slice(0, 500) });
      }
    });
  });
}

async function getBrainExplorerOverview() {
  const result = await runProjectBrainQuery('overview');
  if (!result?.ok) return result;
  return {
    ...result,
    querySource: 'Phase 3 Query Engine',
    queryEngineInstalled: true
  };
}

async function searchProjectBrain(query, mode = 'search', limit = 200) {
  const safeLimit = Math.max(1, Math.min(Number(limit || 200), 500));
  let command = 'search';
  let commandQuery = query;
  if (mode === 'unknown') { command = 'unknown'; commandQuery = null; }
  if (mode === 'unresolved') { command = 'unresolved'; commandQuery = null; }
  if (mode === 'orphans') { command = 'orphans'; commandQuery = null; }

  const raw = await runProjectBrainQuery(command, commandQuery, { limit: safeLimit });
  ...
  if (mode === 'search') {
    rows = rows.map(item => {
      ...
      if (item.relationship) {
        ...
        return {
          path: relation.sourcePath || relation.targetPath || '',
          owner: relation.sourceOwner || relation.targetOwner || 'Unknown',
          state: relation.resolution || 'relationship',
          sizeBytes: 0,
          description: `${relation.relationType || 'reference'} → ${relation.specifier || relation.targetPath || ''}`
        };
      }
      return item;
    });
    ...
    rows = rows.map(item => ({
      ...
      state: 'unresolved',
      sizeBytes: 0,
      description: `${item.relationType || 'reference'} → ${item.specifier || 'unknown target'}`
    }));
  }
  ...
  for (const item of rows) {
    ...
    const key = `${String(item.path).toLowerCase()}\u0000${item.description || ''}`;
    if (!unique.has(key)) unique.set(key, item);
  }
  const results = [...unique.values()].slice(0, safeLimit).map(item => ({
    path: item.path,
...
async function getProjectBrainFile(query) {
  const result = await runProjectBrainQuery('explore-file', query, { depth: 4 });
  if (!result?.ok) return result;
  return {
    ...result,
    querySource: 'Phase 3 Query Engine'
...
ipcMain.handle('get-project-brain', () => getBrainExplorerOverview());
ipcMain.handle('search-project-brain', (_event, query, mode, limit) => searchProjectBrain(query, mode, limit));
ipcMain.handle('get-project-brain-file', (_event, query) => getProjectBrainFile(query));
ipcMain.handle('open-project-brain-folder', () => { fs.mkdirSync(PROJECT_BRAIN_DIR, {recursive:true}); shell.openPath(PROJECT_BRAIN_DIR); return PROJECT_BRAIN_DIR; });

ipcMain.handle('create-forge-test-package', async () => {
  ensureDirs();
  try {
    const sourceVersion = app.getVersion();
    const testVersion = `${sourceVersion}-workflow-test-${new Date().toISOString().replace(/\D/g, '').slice(0, 14)}`;
    const stage = path.join(FORGE_STAGING, `studio-builder-${crypto.randomBytes(8).toString('hex')}`);
    const payload = path.join(stage, 'payload');
    fs.mkdirSync(payload, { recursive: true });

    const required = ['package.json', 'main.js', 'preload.js', 'renderer.js', 'index.html', 'styles.css'];
    for (const name of required) {
      const source = path.join(__dirname, name);
      if (!fs.existsSync(source)) throw new Error(`Current Studio file is missing: ${name}`);
      fs.copyFileSync(source, path.join(payload, name));
    }

    fs.writeFileSync(path.join(payload, 'FORGE_UPDATE_TEST_SUCCESS.txt'),
      `FriendshipTree Forge workflow test\nCandidate version: ${testVersion}\nSource version: ${sourceVersion}\nCreated: ${new Date().toISOString()}\n`,
      'utf8');

    const checksums = {};
    for (const file of walkForHash(payload)) {
      const relative = path.relative(payload, file).replace(/\\/g, '/');
    ...
    const manifest = {
      ...
      author: 'FriendshipTree Studio',
      minimumForgeVersion: '2.1.0',
      description: 'Harmless package-manager workflow test generated by Studio.',
      changelog: [
        'Creates a new isolated candidate version',
        'Adds FORGE_UPDATE_TEST_SUCCESS.txt',
        'Leaves the confirmed working version untouched'
      ],
      sourceVersion,
    ...
    const output = path.join(FORGE_INBOX, `FriendshipTree-Studio-${testVersion}.ftupdate`);
    const escapedStage = stage.replace(/'/g, "''");
    const escapedOutput = output.replace(/'/g, "''");
    const ps = `$ErrorActionPreference='Stop'; Add-Type -AssemblyName System.IO.Compression.FileSystem; if(Test-Path -LiteralPath '${escapedOutput}') { Remove-Item -Force -LiteralPath '${escapedOutput}' }; [System.IO.Compression.ZipFile]::CreateFromDirectory('${escapedStage}','${escapedOutput}',[System.IO.Compression.CompressionLevel]::Optimal,$false)`;
    const result = await invokePowerShell(ps);
    if (!result.ok) throw new Error(result.stderr || 'Could not create the Forge test package.');

    fs.rmSync(stage, { recursive: true, force: true });
    logEvent('forge_test_package.create', 'success', { output, version: testVersion });
    return { ok: true, path: output, version: testVersion };
  } catch (err) {
    logEvent('forge_test_package.create', 'error', { error: err.message });
    return { ok: false, message: err.message };
  }
});
...
function runGit(args) {
  return new Promise(resolve => {
    const child = spawn('git.exe', args, { cwd: PROJECT, shell: false, windowsHide: true });
    let stdout = '';
    let stderr = '';
    child.stdout.on('data', data => { stdout += data.toString(); });
    child.stderr.on('data', data => { stderr += data.toString(); });
    child.on('error', error => resolve({ ok: false, code: -1, stdout, stderr, message: error.message }));
    child.on('close', code => resolve({ ok: code === 0, code, stdout: stdout.trim(), stderr: stderr.trim(), message: code === 0 ? '' : (stderr.trim() || stdout.trim() || `Git exited with code ${code}`) }));
  });
}

async function getGitOverview() {
  const [status, branch, remote, counts, log] = await Promise.all([
    runGit(['status', '--porcelain=v1', '--branch']),
    runGit(['branch', '--show-current']),
    runGit(['remote', 'get-url', 'origin']),
    runGit(['rev-list', '--left-right', '--count', 'HEAD...@{upstream}']),
    runGit(['log', '-10', '--date=short', '--pretty=format:%h%x1f%ad%x1f%an%x1f%s'])
  ]);
  if (!status.ok) return { ok: false, message: status.message || 'Unable to read Git status.' };
  const lines = status.stdout.split(/\r?\n/).filter(Boolean);
  const changes = lines.slice(1).map(line => ({ code: line.slice(0, 2), path: line.slice(3) }));
  const countParts = counts.ok ? counts.stdout.trim().split(/\s+/).map(Number) : [0, 0];
  ...
  return {
    ...
    ahead: Number.isFinite(countParts[0]) ? countParts[0] : 0,
    behind: Number.isFinite(countParts[1]) ? countParts[1] : 0,
...
ipcMain.handle('git-overview', () => getGitOverview());
ipcMain.handle('git-push', async () => {
  const result = await runGit(['push', 'origin', 'main']);
  logEvent('git.push', result.ok ? 'success' : 'error', { message: result.message || result.stdout });
  return result;
});
ipcMain.handle('git-pull', async () => {
  const status = await getGitOverview();
  if (!status.ok) return status;
  if (!status.clean) return { ok: false, message: 'Pull blocked because the working tree has uncommitted changes.' };
  const result = await runGit(['pull', '--ff-only', 'origin', 'main']);
  logEvent('git.pull', result.ok ? 'success' : 'error', { message: result.message || result.stdout });
  return result;
});
ipcMain.handle('open-github', async () => {
  await shell.openExternal('https://github.com/JosephD-93/FriendshipTree');
  return { ok: true };
});
...
app.whenReady().then(() => {
  ...
  // Logging, environment inspection and the inbox watcher are important but do
  // not need to block creation of the first window.
  setTimeout(() => {
    try { startSession(); } catch (error) { console.error('Studio session startup failed:', error); }
    try { startUpdateInboxWatcher(); } catch (error) { console.error('Update watcher startup failed:', error); }
  }, 300);
});
app.on('before-quit', () => { try { updateInboxWatcher?.close(); } catch {} logEvent('session.end', 'success', {}); });
