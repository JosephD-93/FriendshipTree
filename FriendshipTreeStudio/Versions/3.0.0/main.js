...
const UPDATE_INBOX_ROOT = path.join(PROJECT, 'Updates');
...
ipcMain.handle('export-ai-sync-package', async () => {
  ...
  try {
    ...
    const syncManifest = {
      ...
      knowledgeFiles: knowledgeFiles.map(file => path.basename(file)),
...
async function exportAIDevelopmentPackage() {
  ...
  const script = `$ErrorActionPreference='Stop'; Compress-Archive -Path '${escapedFolder}\\*' -DestinationPath '${escapedZip}' -Force`;
  const result = await invokePowerShell(script);
