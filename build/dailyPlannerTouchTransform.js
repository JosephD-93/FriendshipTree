export function dailyPlannerTouchTransform(){
  return {
    ...
    transform(code,id){
      ...
      const replacement='onPointerDown:e=>{const el=e.currentTarget,n=ix(e);if(e.pointerType!=="touch"){if(e.cancelable)e.preventDefault();el.setPointerCapture(e.pointerId);const next={date,a:n,b:n,id:e.pointerId,active:true};dragRef.current=next;setDrag(next);return}const pending={date,a:n,b:n,id:e.pointerId,active:false,startY:e.clientY,timer:null};pending.timer=setTimeout(()=>{if(dragRef.current!==pending)return;pending.active=true;try{el.setPointerCapture(e.pointerId)}catch{};setDrag({...pending})},300);dragRef.current=pending},onPointerMove:e=>{const cur=dragRef.current;if(!cur||cur.id!==e.pointerId)return;if(!cur.active){if(Math.abs(e.clientY-cur.startY)>10){clearTimeout(cur.timer);dragRef.current=null}return}if(e.cancelable)e.preventDefault();const next={...cur,b:ix(e)};dragRef.current=next;setDrag(next)},onPointerUp:e=>{const cur=dragRef.current;if(!cur||cur.id!==e.pointerId)return;clearTimeout(cur.timer);if(!cur.active){dragRef.current=null;return}if(e.cancelable)e.preventDefault();const lo=Math.min(cur.a,cur.b),hi=Math.max(cur.a,cur.b)+1;setDraft({date,title:"",notes:"",location:"",peopleIds:[],color:DEFAULT_EVENT_COLOR,startTime:tm(lo),endTime:hi===48?"23:59":tm(hi)});dragRef.current=null;setDrag(null)},onPointerCancel:e=>{const cur=dragRef.current;if(cur?.id===e.pointerId){clearTimeout(cur.timer);dragRef.current=null;setDrag(null)}},onContextMenu:e=>e.preventDefault(),style:';
