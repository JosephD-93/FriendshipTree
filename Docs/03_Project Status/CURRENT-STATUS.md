...
The current source version is Android `4.3.2` (`versionCode 6`). Settings now contains an App information card, expanded by default, with the version, build identity, build date, platform, What’s new text and a Copy version information button. This is the visible marker for a controlled GitHub cloud-update test: the phone must remain on 4.3.1 until the 4.3.2 APK has been built and downloaded from GitHub. The health widget retains the 4.3.1 closely packed circular controls and responsive layout. Source validation is required; the GitHub build and phone update remain **NOT VERIFIED**.
